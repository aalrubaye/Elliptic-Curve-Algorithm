...
class Decoder {
        ...
        int offset;
        int length;
        public static BigInteger[] decmp = new BigInteger[4];
        ...
        public BigInteger[] decompress(Encoder enc){
        	String[] pp=new String [4];
        	pp[0] = "";
        	for (int i=2; i<enc.lent[0]+2; i++){
        		pp[0] += enc.decmp[0].charAt(i);
        	}
        	pp[1]="";
        	for (int i=enc.lent[0]+2; i<enc.decmp[0].length(); i++){
        		pp[1] += enc.decmp[0].charAt(i);
        	}
        	pp[2] = "";
        	
        	if (enc.lent[1] > 2){
        	for (int i=2; i<enc.lent[1]+2; i++){
        		pp[2] += enc.decmp[1].charAt(i);
        	}
        	pp[3]="";
        	for (int i=enc.lent[1]+2; i<enc.decmp[1].length(); i++){
        		pp[3] += enc.decmp[1].charAt(i);
        	}
        	}
        	else{
        		pp[2] = "0";
        	...
        	for (int j=0; j<4; j++)
        	decmp[j] = new BigInteger(pp[j],16);
